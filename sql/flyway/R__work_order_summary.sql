...
WITH per_work_order AS (
  SELECT work_order_id,
    MAX_BY(client_id, updated_at_utc) AS client_id, MAX_BY(store_id, updated_at_utc) AS store_id,
    MAX_BY(technician_id, updated_at_utc) AS technician_id, MAX_BY(priority, updated_at_utc) AS priority,
    ...
    COUNT(*) AS event_count, COUNT_IF(event_type = 'REOPENED') AS reopen_count,
    MAX(updated_at_utc) AS last_updated_timestamp
  FROM SILVER_WORK_ORDER_EVENTS GROUP BY work_order_id
)
SELECT work_order_id, client_id, store_id, technician_id, priority, opened_timestamp, closed_timestamp,
  CASE last_event_type WHEN 'CLOSED' THEN 'CLOSED' WHEN 'REOPENED' THEN 'REOPENED'
    WHEN 'OPENED' THEN 'OPEN' WHEN 'WORK_COMPLETED' THEN 'COMPLETED' ELSE last_event_type END AS current_status,
